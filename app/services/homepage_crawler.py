...
async def crawl_homepages(crawler: AsyncWebCrawler, sources: List[Dict], urls: List[str], query: str, semaphore) -> List[Dict]:
    """Crawl homepages for article links."""
    scrape_articles = []
    for source in sources:
        ...
        try:
            config = SITE_CONFIGS.get(source['name'], {})
            link_selector = config.get("selectors", {}).get("title", ".entry-title a, .td-post-title a, .post-title a, h3 a, .td_module_wrap a, .tdb-block-inner a, .td-block-span12 a, .td-block-span6 a")
                ...
                for link in raw_links[:50]:
                    ...
                    if ('category' in href.lower() or
                        ...
                        any(keyword in href.lower() for keyword in [
                            'about-us', 'afaanoromoo', 'amharic', 'wp-login',
                            'contact-us', 'journal.addisstandard', 'peptide',
                            'roasted-garlic', 'exodus', 'women-at-the-top', 'advertisement'
                        ]) or
            ...
            logger.error(f"Failed to scrape homepage {source['homepage']}: {e}")
